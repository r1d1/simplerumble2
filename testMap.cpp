...
{
	sf::RenderWindow App(sf::VideoMode(WINDOW_WIDTH, WINDOW_HEIGHT), "Map test");
	//sf::Style::Fullscreen

	sf::Image bgMap;
	bgMap.create(400, 300, sf::Color::Black);
	for(int i=0 ; i < int(0.01 * bgMap.getSize().x * bgMap.getSize().y) ; i++){ int xr=rand() % bgMap.getSize().x; int yr=rand() % bgMap.getSize().y; bgMap.setPixel(xr, yr, sf::Color::Blue); }
//	for(int i=0 ; i < int(0.8 * bgMap.getSize().x * bgMap.getSize().y) ; i++){ int xr=exp(-pow((rand() % bgMap.getSize().x-150),2)); int yr=exp(-pow((rand() % bgMap.getSize().y - 200),2)); bgMap.setPixel(xr, yr, sf::Color::Blue); }
//	for(int i=0 ; i < int(0.3 * bgMap.getSize().x * bgMap.getSize().y) ; i++){ int xr=exp(-pow((rand() % bgMap.getSize().x-150),2)); int yr=exp(-pow((rand() % bgMap.getSize().y - 200),2)); bgMap.setPixel(xr, yr, sf::Color::Green); }
//	for(int i=0 ; i < int(0.1 * bgMap.getSize().x * bgMap.getSize().y) ; i++){ int xr=exp(-pow((rand() % bgMap.getSize().x-150),2)); int yr=exp(-pow((rand() % bgMap.getSize().y - 200),2)); bgMap.setPixel(xr, yr, sf::Color::Red); }
	sf::Texture bgTexture;
	bgTexture.loadFromImage(bgMap);
	sf::Sprite bgSprite;
