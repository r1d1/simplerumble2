...
int main(int argc, char** argv)
{
	...
	while (App.isOpen())
	{
		...
		while (App.pollEvent(event))
		{
            ...
            {
                if(prev_abs_x >= 0 and prev_ord_y >= 0){ testMap.setTileType(prev_abs_x, prev_ord_y, previousType); }
                cursorPos = sf::Mouse::getPosition(App);
                abs_x = cursorPos.x / testMap.getTileWidth();
                ord_y = cursorPos.y / testMap.getTileHeight();
                std::cout << abs_x << "," << ord_y << std::endl;
                if(abs_x < testMap.getWidth() and ord_y < testMap.getHeight() and abs_x > 0 and ord_y > 0)
                {
                    previousType = testMap.getTileType(abs_x, ord_y);
                    prev_abs_x = abs_x;
                    prev_ord_y = ord_y;
                    testMap.setTileType(abs_x, ord_y, 'x');
                }
            }
            if(sf::Mouse::isButtonPressed(sf::Mouse::Button::Left))
            {
                if(abs_x < testMap.getWidth() and ord_y < testMap.getHeight() and abs_x > 0 and ord_y > 0)
                {
                    ...
                    abs_x = cursorPos.x / testMap.getTileWidth();
                    ord_y = cursorPos.y / testMap.getTileHeight();
                    std::cout << testMap.getTileType(abs_x, ord_y) << std::endl;
                }
            }
